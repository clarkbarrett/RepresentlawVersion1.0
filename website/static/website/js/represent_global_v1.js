...
2. represent functions
----------------------------
eg represent.changeLocation(para1,para2);
a. changeLocation
b. getQueryVariable
c. AJAX search
...
// 2. Set represent object and domain
var represent = {};

// 2a. changeLocation
(function($) {
  if(typeof represent.changeLocation == "undefined") represent.changeLocation = {
    init:function() {
      $("#location a").click(function() {
        if ($(this).html() == "Scotland"){
          represent.changeLocation.change('england','scotland');
        } else {
          represent.changeLocation.change('scotland','england');
        }
      });
    },
...
// 2b. getQueryVariable
if(typeof represent.getQueryVariable == "undefined") represent.getQueryVariable = {
...
// 2c. AJAX search
(function($) {
    if(typeof represent.liveSearch == "undefined") represent.liveSearch = {
        ...
        init: function() {
            ...
            $("#queries_searchfield_query-new").bind('keyup', function() {
                ...
                self.searchTimeout = setTimeout(function() {
                    ...
                    represent.liveSearch.ajaxSearch();
                    }, 200 // increase time to reduce the server load
                );
            }).focus(function() {
                represent.liveSearch.clearField(this.id);
            }).blur(function() {
                represent.liveSearch.restoreField(this.id);
            });

            // when the return/enter key is pressed
            $(document).keyup(function(event){
                if (event.keyCode == 27) {
                    represent.liveSearch.closeSearchList();
                }
            });
    ...
    ajaxSearch: function() {
      ...
      if (!self.doingAjaxSearch && (self.currentSearchTerm != self.search_term) && (self.search_term != "Search represent...")) {
        if (search_term == "") {
          $("#search_list").fadeOut("medium");
        } else if (search_term.length >= 3) {
          represent.liveSearch.doSearch();
        }
      }
    }, // end ajaxSearch
    ...
    closeSearchList: function() {
      ...
      $("#search_list").fadeOut("medium");
      clearTimeout(represent.liveSearch.searchTimeout);
      $("#queries_searchfield_query-new").removeClass("active_search");
      return false;
    }, // end closeSearchList
...
/*(function($) {
  ...
  if ($(this).attr("onclick") == undefined) {
    var url = document.location.toString().split("represent.org.uk")[1] + '/thank_you';
...
if (typeof represent.countryInfo == "undefined") represent.countryInfo = {
...
}; // end represent.countryInfo


// 2f. Table of contents
if (typeof represent.innerContentPageNav == "undefined") represent.innerContentPageNav = {
...
}; // end represent.innerContentPageNav
...
/*
  ...
  represent.inlineFormValidate.init({
    formID:"form_email_39309",
    fields: fieldsToCheck
  });
*/
if(typeof represent.inlineFormValidate == "undefined")  represent.inlineFormValidate = {
  // standard regexp
  regExp : {
    "textBox":"^[^\\\\\"]{1,300}$",
    "textArea": "^.{1,3000}",
  ...
  init: function(params) {
    ...
    $("#" + this.formID + " :input, #" + this.formID + ":input:radio").blur(function() {
      represent.inlineFormValidate.doInlineCheck($(this).attr("id"));
    });
    ...
    $("#optIn_email, #optIn_sms").click(function() {
      ...
      represent.inlineFormValidate.optInOutHandler($(this).attr("id"), type + "OptIn");
    });
    ...
    $("#" + this.formID).submit(function(event) {
      ...
      $("#" + represent.inlineFormValidate.formID + " :input, #" + represent.inlineFormValidate.formID + ":input:radio").each(function() {
        represent.inlineFormValidate.doInlineCheck($(this).attr("id"));
      });

      // Check for uncorrected errors
      if ($("#" + represent.inlineFormValidate.formID + " div, #" + represent.inlineFormValidate.formID + " li").is(".requiredError, .requiredErrorWrapper")) {
        $("#formErrors").html("<div style=\"color:#f00;\"><strong>Please correct the information in the fields below.</strong></div>");
        $("#" + represent.inlineFormValidate.formID).find('.requiredError:first').scrollTo();
        event.preventDefault();
      } else {
         // $("#" + represent.inlineFormValidate.formID + "_submit").val("Please wait...");
        return true;
      }
    });
  },
        ...
        case "text":
          ...
          if (fieldProperties.regExp == "postcode" && country != "") {
            if ($("#" + country + " option:selected").val() == "gb") {
              represent.inlineFormValidate.checkField(fieldID, this.regExp[fieldProperties.regExp], fieldProperties.msg, fieldProperties.req);
            } else {
              represent.inlineFormValidate.displayMsg(fieldID, "#" + fieldID, "", false);
            }
          } else {
            represent.inlineFormValidate.checkField(fieldID, this.regExp[fieldProperties.regExp], fieldProperties.msg, fieldProperties.req);
          }
          break;
        case "dropdown":
          represent.inlineFormValidate.checkDropDown(fieldID, fieldProperties.emptyValue, fieldProperties.msg, fieldProperties.req);
          break;
        case "radio":
          represent.inlineFormValidate.checkRadioSelect(fieldID, fieldProperties.msg);
          break;
        case "dob": // has a weird id that would require using eval - BAD
          represent.inlineFormValidate.checkDOB(fieldProperties.day, fieldProperties.month, fieldProperties.year, fieldProperties.minAge, fieldProperties.req, "");
          break;
        case "date":
          represent.inlineFormValidate.checkDOB(fieldProperties.day, fieldProperties.month, fieldProperties.year, fieldProperties.minAge, fieldProperties.req, fieldProperties.msg);
          break;
        case "title":
          represent.inlineFormValidate.checkTitle(fieldID, fieldProperties.titleOther);
          break;
        case "checkbox":
          represent.inlineFormValidate.checkCheckConfirm(fieldID, fieldProperties.msg);
          break;
        case "confirmEmail":
          represent.inlineFormValidate.checkField(fieldID, this.regExp["email"], fieldProperties.msg, fieldProperties.req);
          represent.inlineFormValidate.checkEmailConfirm(fieldProperties.emailID, fieldID);
          break;
        case "creditcard":
          represent.inlineFormValidate.checkCardNumberField(fieldID, fieldProperties.msg, fieldProperties.req);
          break;
      }
    }
  },  // end doInlineCheck
  ...
  displayMsg: function(field, fieldID, msg, show) {
    ...
    if (represent.inlineFormValidate.newDonateForm == true) {
      if (show) {
  ...
  checkField: function (field, regExCheck, message, required) {
    ...
    if (regExLine.test(fieldVal)) {
      ...
      if (fieldVal != "" || (fieldVal == "" && required == 1)) {
        represent.inlineFormValidate.displayMsg(field, fieldID, message, true);
      } else {
        represent.inlineFormValidate.displayMsg(field, fieldID, message, false);
      }
    }
  ...
  checkExpiryDateField: function (monthField, yearField, regExCheckMonth, regExCheckYear, message, required) {
    ...
    if (regExLineMonth.test(monthFieldVal) && regExLineYear.test(yearFieldVal)) {
      ...
      if ((monthFieldVal != "" || yearFieldVal != "") || (monthFieldVal == ""  && yearFieldVal == "" && required == 1)) {
        represent.inlineFormValidate.displayMsg(yearField, fieldID, message, true);
      }
    }
  ...
  checkCardNumberField: function (field, message, required) {
    ...
    if (total % 10 == 0 && fieldVal.length >= 12 && fieldVal.length <= 19) {
      if (fieldVal != "" || (fieldVal == "" && required == 0)) {
        ...
        represent.inlineFormValidate.displayMsg(field, fieldID, "", false);
      } else {
        $(fieldID).parent().removeClass("requiredError requiredOkay");
      }
      if ($(fieldID + "_reqMsg").length > 0) {
  ...
  checkTitle: function(title, titleOther) {
    ...
    if (titleField.val() != " ") {
      ...
      represent.inlineFormValidate.displayMsg(title, "#" + title, "Please select a title", true);
    }
  }, // end checkTitle
  ...
  checkEmailConfirm: function(email1, email2) {
    ...
    if (emailField.val() != emailConfirmField.val()) {
      represent.inlineFormValidate.displayMsg(email2 + "_email", "#" + email2, "Your email addresses don't match", true);
    } else {
  ...
  checkCheckConfirm: function (fieldID, message) {
    ...
    if (checkBoxField.attr("checked") == "" || checkBoxField.attr("checked") == undefined) {
      represent.inlineFormValidate.displayMsg(fieldID, "#" + fieldID, message, true);
      return false;
    } else {
      represent.inlineFormValidate.displayMsg(fieldID, "#" + fieldID, message, false);
      return true;
    }
...
if (typeof represent.bankwizard == "undefined") represent.bankwizard = {
  // for bank wizard
  branchdetails: {},
  bankvalid: true,
  ...
  check: function(accNo, sortCode, functionToCall) {
    var bankwizardreached = function(json) {
      if (!json.validated) {
        ...
        represent.bankwizard.bankvalid = false;
      }
      if (json.branchdetails) {
        represent.bankwizard.bankvalid = true;
        errorMessage = "";
        represent.bankwizard.branchdetails = json.branchdetails;

        $("#step3 span#bankName_answer").html(represent.bankwizard.branchdetails.bankname);
      }
 ...
 **/
(function ($) {
    if (typeof represent.addressLookUp == "undefined") represent.addressLookUp = {
        ...
        callback: "represent.addressLookUp.finishedLookUp",
        ...
        init: function (parameters) {
            ...
            $("#" + this.submitButtonID).bind("click", $("#lookup_postcode"), function (e) {
                represent.addressLookUp.clearAddressErrors();
                represent.addressLookUp.PostcodeAnywhere_getAddressList();
            });
            ...
            $("#enterAddress").click(function (event) {
                ...
                $(represent.addressLookUp.addressFieldsWrapper).slideDown(500);
        ...
        PostcodeAnywhere_getAddressList: function() {
            ...
            url += "&Key="      + represent.addressLookUp.query_key;
            url += "&Postcode=" + encodeURIComponent( inputPostcode );
            url += "&UserName=" + represent.addressLookUp.query_username;
            url += "&CallbackFunction=represent.addressLookUp.PostcodeAnywhere_getAddressList_callback";
        ...
        PostcodeAnywhere_getAddressList_callback: function(response) {
            ...
            else
            {
                ...
                {
                    this.displayMessage(represent.addressLookUp.noAddressMsg);
                }
                else if( response.length == 1 )
                {
                    represent.addressLookUp.PostcodeAnywhere_getAddressById( response[0].Id );
        ...
        PostcodeAnywhere_getAddressById: function( Id ) {
            ...
            url += "&CallbackFunction=represent.addressLookUp.PostcodeAnywhere_getAddressById_Callback";
        ...
        PostcodeAnywhere_getAddressById_Callback: function(response) {
            ...
            if (response.length == 1 && typeof(response[0].Error) != "undefined")
            {
                $(represent.addressLookUp.inputPostcodeID).removeClass("ac_loading");
                represent.addressLookUp.displayMessage(this.noAddressMsg);
            }
            else
            {
                // Check if there were any items found
        ...
        displayList: function () {
            ...
            // Append that to the DOM
             ...
             $('select[name="address_selection"]').change(function(e){
                ...
                represent.addressLookUp.PostcodeAnywhere_getAddressById( addressId );
             });
        }, // end displayList
...
// 2i. Twitter
if (typeof represent.twitterParser == "undefined") represent.twitterParser = {
  init:function() {
    ...
    $(".tweet").each(function() {
      ...
      // remove the 'represent:' prefix
      tweetLine = tweetLine.replace(/represent: /i, "");
...
(function($) {
if (typeof represent.animated_banner == "undefined") represent.animated_banner = {
  ...
  init:function(parameters) {
    ...
    represent.animated_banner.get_data();
  },
    ...
    $.ajax({
        ...
        success: function(data){
          ...
          if (count == arrayLength)
          {
            ...
            represent.animated_banner.start_animation();
            represent.animated_banner.stopStartRotate();
          } else {

            represent.animated_banner.incrementCount();
            represent.animated_banner.get_data();
          }
        }
      });
  ...
  stopStartRotate:function()
  {
    $("#" +  this.containerDiv).mouseenter(function(){
      represent.animated_banner.stop_animation();
    });

    $("#" +  this.containerDiv).mouseleave(function(){
      represent.animated_banner.restart_animation();
    });
  },
  ...
  start_animation:function()
  {
    ...
    this.bannerTimer = setTimeout(function(){represent.animated_banner.show_next_panel(this.current_panel,this.next_panel);}, this.panelTime);
  },
...
//2t. Cookie bar
(function($) {
  if (typeof represent.cookiebar == "undefined") represent.cookiebar = {
    init: function() {
...
(function($) {
  ...
  if (typeof represent.lightbox_survey == "undefined") represent.lightbox_survey = {

    init: function() {

      var currentURI = window.location.toString();
      // represent.domain check removed as it fails - does not register england but www instead
      ...
      //if ((represent.domain == 'england') && (document.location.toString().indexOf('get_advice') != -1)) {
      if ((currentURI.indexOf('england') != -1) && (document.location.toString().indexOf('get_advice') != -1)) {
        ...
        if (!y) {
          ...
          var lightbox_survey = '<div id="lightbox_survey_wrapper"><div id="lightbox_survey_inner" class="clearfix"><h2>Could you take a minute to help us improve our website?&nbsp;&nbsp;&nbsp;&nbsp;<a href="//england.represent.org.uk/get_advice/advice_survey_2015?referer=' + encodeURI(document.location) + '" id="advice_survey">Yes</a> / <a href="#" id="lightbox_survey_close">No</a></h2></div></div>';
...
(function($) {
  ...
  if (typeof represent.website_survey == "undefined") represent.website_survey = {
...
(function($) {
  jQuery(document).ready(function($) {
        ...
        switch(document.body.id) {
          case 'represent-england-body':
            represent.domain = 'england';
            break;
          case 'represent-scotland.body':
            represent.domain = 'scotland';
            break;
          case 'represent-neutral-body':
            represent.domain = 'www';
            break;
          case 'represent-media-body':
            represent.domain = 'media';
            break;
          default:
            represent.domain = 'www';
        }
    ...
    represent.cookiebar.init();
    represent.lightbox_survey.init();
    //represent.website_survey.init();

    // add page binds here or on in page content
    var adviceTab = new represent.megaMenu();
    var proTab = new represent.megaMenu();
    var wycdTab = new represent.megaMenu();
    var campTab = new represent.megaMenu();

    var adviceTabId = (document.domain == "scotland.represent.org.uk") ? "nav11811" : "nav24462";
    var proTabId = (document.domain == "scotland.represent.org.uk") ? "nav12107" : "nav24522";
    var wycdTabId = (document.domain == "scotland.represent.org.uk") ? null : "nav24466";
    var campTabId = (document.domain == "scotland.represent.org.uk") ? null : "nav417501";
    ...
    represent.social.loadAPIs();
    //represent.liveSearch.init();
    represent.changeLocation.init();

    represent.makeComboBox.init();
